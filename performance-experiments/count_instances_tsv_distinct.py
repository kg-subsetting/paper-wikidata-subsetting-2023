...
def count_instances(tsv_file):
    ...
    genes = df.loc[(df['node2'].eq('Q7187')) & (df['id'].str.contains('-P31-Q'))]['node1'].unique().tolist()
    proteins = df.loc[(df['node2'].eq('Q8054')) & (df['id'].str.contains('-P31-Q'))]['node1'].unique().tolist()
    chemicals = df.loc[(df['node2'].eq('Q11173')) & (df['id'].str.contains('-P31-Q'))]['node1'].unique().tolist()
    diseases = df.loc[(df['node2'].eq('Q12136')) & (df['id'].str.contains('-P31-Q'))]['node1'].unique().tolist()
